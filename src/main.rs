...
fn model(app: &App) -> Model {
    ...
    Model {
        ...
        past_magnitudes: vec![vec![0.0; 100]; 6],
...
fn update(_app: &App, model: &mut Model, _update: Update) {
    ...
    let neon_hue = 0.4 + 0.3 * (model.hue / 1.0);

    model.line_color = nannou::color::hsl(neon_hue, 1.0, 0.5).into();
    ...
    let amplitude = if volume > 0.0 {
        (volume.log(10.0) * 140.5).max(1.0).min(100.0)
    } else {
        1.0
    };
    ...
    let num_points = 2000;
    let frequency = frequency_multiplier * 5.0; 
    ...
    let spectral_flux_frames: usize = 100; // Number of past frames to average
    ...
    let mut target_circle_radius = 90.0;
    let beat_detection_threshold = 0.5;
    const COOLDOWN_TIME: usize = 24;
    if model.cooldown_counter == 0 {
        if avg_spectral_flux > beat_detection_threshold {
            model.hue = (model.hue + 0.9) % 1.0;
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    // let mut string_positions: [f32; 50] = arr.map(|x| x + 1.0);

    let string_positions: [i32; 50] = (1..=50).collect::<Vec<_>>()
    ...
    for (index, &position) in string_positions.iter().enumerate() {
        if index < model.string_points.len() {
            ...
            for &point in points.iter() {
                let x = point.x;
                let y = point.y + position as f32 * 10.0 - 150.0; 
    ...
    for (index, &position) in string_positions.iter().enumerate(){
    draw.ellipse()
        .x_y(position as f32 * 50.0 - 1000.0  + model.past_spectral_flux[0], 
            index as f32  * 10.0 + model.past_spectral_flux[0] - 50.0)
        .radius(model.circle_radius * 3.0)
        .color(circle_color);
    }
